...
export function buildCalendarUpdateRulesEntryContent(): string {
  ...
  return [
    ...
    "          重复规则: optional['每天' | '每周' | '每月' | '每年' | '仅工作日' | '仅节假日'];",
    '          提前提醒天数: optional[number];',
    "          可见性: optional['玩家与LLM' | '完全不显示' | '仅玩家'];",
    "          类型: optional['日程' | '回忆'];",
    '          标签: optional[string[]];',
    ...
    '      - 普通日程默认`玩家与LLM`，可省略`可见性`',
    '      - 已确定但暂时不应被玩家或LLM提前知道的未来时间事实可写`完全不显示`；它必须来自已有因果或明确规则，不得由月历自行策划',
    '      - `完全不显示`只能由月历脚本在提醒时间提升为`仅LLM`；LLM不得主动写`仅LLM`',
    '      - 看到系统提供的`仅LLM`事件时，只判断玩家是否已经通过剧情获知；获知后可改为`玩家与LLM`，否则保持不变',
    '      - `回忆`只用于已经发生的过去记录，必须`可见性: 仅玩家`；它是给玩家在UI回看的只读历史卡，不参与任务、世界状态或剧情触发',
    '      - 已发生且仍值得玩家回看的普通日程可转为`回忆`；否则事件失去提醒价值后移除',
